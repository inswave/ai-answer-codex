...
(async () => {
  ...
  try {
    ...
    await sleep(9000);
    // 첫 AI 답변 thread item (헤더 'AI 재답변' 라벨 + 본문 포함) 우선, 없으면 ai-answer-body
    const handle = await page.evaluateHandle(() => {
      const item = document.querySelector('.ai-thread-item.ai-reply') || document.querySelector('.ai-answer-body');
      return item || null;
    });
    ...
    if (el) {
      ...
      console.log('크롭 완료 |', box && (Math.round(box.width)+'x'+Math.round(box.height)));
    } else console.log('요소 못 찾음');
