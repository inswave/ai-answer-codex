...
/**
 * LLM CLI 백엔드를 자식 프로세스로 실행하고 최종 답변 텍스트를 반환한다.
 * 두 출력 모드를 지원:
 *  - 'lastMessageFile' (codex exec): `--output-last-message <임시파일> -`로 stdin 입력,
 *    종료 후 임시파일에서 최종 메시지를 읽는다.
 *  - 'stdout' (claude -p): stdin으로 프롬프트를 넣고 stdout으로 나온 최종 답변을 읽는다.
 *
 * @param {object} runner - { name, command, args, env, outputMode }
 * @param {string} input - LLM에 전달할 전체 프롬프트
 * @param {number} timeoutMs
 */
function callRunner(runner, input, timeoutMs) {
  return new Promise((resolve, reject) => {
    const label = runner.name || runner.command;
    const useFile = runner.outputMode === 'lastMessageFile';
    const outputPath = useFile
      ? path.join(os.tmpdir(), `techassistant-llm-${process.pid}-${Date.now()}.txt`)
      : null;
    const args = useFile
      ? [...(runner.args || []), '--skip-git-repo-check', '--output-last-message', outputPath, '-']
      : [...(runner.args || [])];
    ...
    child.on('close', (code) => {
      ...
      if (code !== 0) {
        cleanup();
        return reject(new Error(`${label} exit ${code}: ${stderr.trim() || stdout.slice(-500)}`));
      }
      try {
        const finalMessage = (outputPath && fs.existsSync(outputPath))
...
/**
 * config 항목으로부터 실행 가능한 runner 설정을 만든다.
 * 'claudeP' → claude -p (stdout 모드), 그 외(codexExec 등) → codex exec (파일 모드).
 * config에 command/args/outputMode를 명시하면 그 값이 우선한다.
 */
function buildRunner(name, c = {}) {
  const isClaude = name === 'claudeP' || c.outputMode === 'stdout';
  const model = c.model || (isClaude ? 'claude-opus-4-8' : 'codex-exec');
  const command = c.command || (isClaude ? 'claude' : 'codex');
  const args = c.args
    || (isClaude
      ? ['-p', '--model', model]
  ...
  return {
    ...
    args,
    env: c.env || {},
    outputMode: c.outputMode || (isClaude ? 'stdout' : 'lastMessageFile'),
...
## 분석 우선 (작성 전 사고)
- 답을 쓰기 전에 먼저: (1) 고객이 진짜 묻는 핵심 1~2개를 정의 → (2) 참고자료가 그 핵심에 대해 "확정해주는 것 / 빈칸"을 구분 → (3) 빈칸은 WebSquare 엔진 동작 원리로 추론해 메웁니다.
- 참고자료를 그대로 나열·요약하지 말고, 이 고객 케이스에 "적용·추론한 결과"를 씁니다.
- 단, API/속성/이벤트/옵션의 "이름"은 추론으로 만들지 않습니다(존재가 확인된 것만). 추론은 동작 방식·적용 방향·구조 설계에만 사용합니다.

## 판단·추천 (나열 금지)
- 가능한 방법이 여러 개면 "A도 되고 B도 됩니다"로 나열하지 않습니다. 고객이 밝힌 제약(보안 정책, 마이그레이션 비용, 유지 방식 등)에 비춰 "이 경우엔 ___을 권장합니다 — 이유는 ___" 으로 하나를 고르고 근거를 댑니다. 나머지 방법은 "B는 ___ 때문에 이 경우엔 덜 맞습니다" 한 줄로 처리합니다.
- 한 발 더: 고객이 잡은 방향이 최선이 아닐 수 있으면, 더 나은 접근이나 놓치기 쉬운 함정 1가지를 짚어줍니다. (단, 고객 질문에 대한 답을 먼저 한 뒤 덧붙입니다.)
...
## 답변 구조
고정 인사말 다음 흐름은 질문에 맞춰 유연하게 씁니다(아래는 골격 예시일 뿐, 기계적으로 빈칸 채우듯 쓰지 않습니다). 단순 질문이면 결론+최소 예시로 짧게, 가능여부/설계 질문이면 결론(가능·조건부가능·불가를 먼저 단정)→근거 추론→권장 구현→확인 필요(있을 때만) 순으로 씁니다.
...
- 확인 요청은 답을 못 쓸 때만, 정말 필요한 1~2개로 한정합니다(체크리스트식 나열 금지). 이미 추론·안내한 내용에 대해 정보를 더 달라고 길게 늘어놓지 않습니다.
...
- 보통 6~12문장 안에서 끝냅니다.
- 확실한 내용은 단정적으로 씁니다. "확인이 필요합니다"는 정말 버전/환경에 의존하는 부분에만 1회 수준으로 쓰고, 결론 자체를 미루는 데 쓰지 않습니다.
...
class AnswerGenerator {
  constructor(config) {
    ...
    // LLM 백엔드 폴백 체인. llmChain(배열)이 우선, 없으면 단일 llmProvider, 둘 다 없으면 codexExec.
    // 앞에서부터 시도하고 실패/빈응답이면 다음으로 폴백한다.
    const chain = Array.isArray(fullConfig.llmChain) && fullConfig.llmChain.length
      ? fullConfig.llmChain
      : [fullConfig.llmProvider || 'codexExec'];

    this.runners = chain.map((name) => buildRunner(name, fullConfig[name] || {}));
    if (!this.runners.length) {
      throw new Error('llmChain이 비어 있습니다. config에 llmChain 또는 llmProvider를 설정하세요.');
    }
    // 첫 백엔드의 모델을 기본 라벨로 노출(실제 사용 모델은 호출 시 결정).
    this.model = this.runners[0].model;
  }

  /**
   * 폴백 체인을 앞에서부터 시도. 비정상 종료/타임아웃/빈 응답이면 다음 백엔드로 넘어간다.
   * @returns {{ text: string, model: string }}
   */
  async _runChain(prompt) {
    let lastErr;
    for (const runner of this.runners) {
      try {
        const raw = await callRunner(runner, prompt, runner.timeoutMs);
        const text = String(raw || '').trim();
        if (text) return { text, model: runner.model };
        lastErr = new Error(`${runner.name} 빈 응답`);
        console.error(`[LLM 폴백] ${runner.name} 빈 응답 → 다음 백엔드 시도`);
      } catch (err) {
        lastErr = err;
        console.error(`[LLM 폴백] ${runner.name} 실패 → 다음 백엔드 시도: ${err.message}`);
      }
    }
    throw new Error(`모든 LLM 백엔드 실패: ${lastErr ? lastErr.message : 'unknown'}`);
  }
  ...
  async _callLLM(systemPrompt, userMessage) {
    ...
    const { text, model } = await this._runChain(prompt);
    return {
      text,
      inputTokens: 0,
      outputTokens: 0,
  ...
  async _ensureKoreanAnswer(text, originalPrompt) {
    ...
    const { text: rewritten } = await this._runChain(rewritePrompt);
