...
const RULES = [
  {
    ...
    pattern: /라이선스|라이센스|license|ellicense|설치\s*파일|websquare2|웹스퀘어\s*2|websquare\s*2|버전\s*내리|키\s*발급|계약|권한\s*확인|위험\s*첨부|데모\s*(?:라이선스|라이센스|버전|판|신청|요청|키|계정|환경)|평가판|체험판|엔진\s*(?:파일|다운로드|받|제공|전달|요청)|플러그인\s*(?:전달|다운로드|파일|받|제공|요청|주세요)/i,
  },
  {
